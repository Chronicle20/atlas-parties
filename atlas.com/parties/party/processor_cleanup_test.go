...
func TestLeaveAndEmit_EdgeCases(t *testing.T) {
	...
	t.Run("successful leave with party disbanding", func(t *testing.T) {
		...
		assert.Equal(t, 0, len(result.Members())) // Party should be disbanded
...
func TestGetByCharacter_EdgeCases(t *testing.T) {
	...
	t.Run("character not found", func(t *testing.T) {
		processor, _, _ := setupCleanupTest()
	...
	t.Run("successful character lookup in party", func(t *testing.T) {
		...
		fn := func(m Model) Model { return Model.AddMember(m, char.Id()) }
		party, _ = GetRegistry().Update(ten, party.Id(), fn)
